...
const byId = (list, id) => list && list.find && list.find(x => x.id === id);

const DATA_ROOTS = ["./assets/data","./main/data"]; // canonical then legacy
async function loadJSON(relPath){
  return await firstOk(DATA_ROOTS.map(root => `${root}/${relPath}`));
}
...
function applyStylepack(packId, packs, doc){
  ...
  const pack = packs && (packs.packs || packs).find(p => p.id === packId);
  ...
  // expose for other engines (AlchemyFusion)
  window.applyStylePack = (target, id) => applyStylepack(id, packs, doc);
}
...
async function mountGeometry(path){
  ...
  img.src = path; img.alt = "Sacred Geometry Scaffold";
...
async function ensureAudio(){
  ...
  comp.connect(convolver); convolver.connect(limiter); limiter.connect(master); master.connect(AC.destination);
  nodes={osc,gain,master,convolver};
  return AC;
}
async function loadIR(){
  if(!AC) await ensureAudio();
  for (const url of ["./assets/audio/ir/cathedral_small.wav","./audio/ir/cathedral_small.wav"]){
    try{
...
async function startTone(hz=defaultHz){ if(!AC) await ensureAudio(); if(!irLoaded) await loadIR(); nodes.osc.frequency.linearRampToValueAtTime(hz, AC.currentTime+.25); fadeTo(0.08); }
...
(async function boot(){
  try{
    ...
    const ov = document.createElement("div"); ov.className="overlay-vitrail"; document.body.appendChild(ov);
    mountPlaque(room);
  } catch(e){ console.error("Cathedral boot error:", e); }
})();